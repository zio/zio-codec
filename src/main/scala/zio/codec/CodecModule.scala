...
trait CodecModule {
  ...
  object Codec {
    ...
    sealed case class Fail[A](error: String) extends Codec[A]
    case object Consume extends Codec[Input]
    sealed case class Map[A, B](equiv: Equiv[A, B], value: () => Codec[A]) extends Codec[B]
    sealed case class Filter[A](value: () => Codec[A], filter: Set[A]) extends Codec[A]
    sealed case class Zip[A, B](left: () => Codec[A], right: () => Codec[B]) extends Codec[(A, B)]
    sealed case class Alt[A, B](left: () => Codec[A], right: () => Codec[B]) extends Codec[Either[A, B]]
    sealed case class Rep[A](value: () => Codec[A], min: Option[Int], max: Option[Int]) extends Codec[Chunk[A]]
  }
}
